...
class ImuBuffer {
private:
    std::deque<std::pair<Eigen::VectorXd, ros::Time>> vio_buffer;
...
public:
    ...
    void addPair(const Eigen::VectorXd& data, const ros::Time& time) {
        ...
        if (vio_buffer.size() >= max_buffer_size) {
            vio_buffer.pop_front();
        }
        vio_buffer.push_back(std::make_pair(data, time));
    }
